...
const productosBD = [
    // Desayunos
    { id: 1, nombre: "Desayuno Típico Hondureño", precio: 96.00, categoria: "desayunos", img: "Desayuno.jpeg" },
    ...
    // Ofertas Comprables
    { id: 101, nombre: "Combo Desayuno Típico + Cappuccino", precio: 120.00, categoria: "ofertas", img: "Desayuno.jpeg" },
...
async function confirmarPedido(event) {
    ...
    const direccion = document.getElementById("txtDireccion") ? document.getElementById("txtDireccion").value : "";
...
async function renderizarHistorial() {
    ...
    try {
        ...
        // Contador visual que empieza en 1 siempre
        let numeroVisual = 1;
        ...
        pedidosRealizados.forEach(p => {
            ...
            // Usamos numeroVisual para la columna 'N°', y agregamos el botón con las comillas simples '${p.id}'
            tr.innerHTML = `
                <td>${numeroVisual}</td>
                <td>${p.cliente}</td>
                <td>${p.destino}</td>
...
async function eliminarPedido(idOriginalBaseDatos) {
    ...
    if(confirmar) {
        try {
            // Codificamos el ID porque contiene el símbolo "#" (ej. #001)
            ...
            if(respuesta.ok) {
                // Volvemos a dibujar la tabla, los números se reacomodarán solos
                renderizarHistorial();
            } else {
